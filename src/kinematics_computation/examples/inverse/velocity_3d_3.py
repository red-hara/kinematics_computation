...
class Robot:
    ...
    def inverse_vel(self, g, translation_vel: vector.Vector3):
        ...
        jacobi = np.transpose(
            np.array(
                [
                    [j0.translation_vel.x, j0.translation_vel.y, j0.translation_vel.z],
                    [j1.translation_vel.x, j1.translation_vel.y, j1.translation_vel.z],
                    [j2.translation_vel.x, j2.translation_vel.y, j2.translation_vel.z],
                ],
                dtype="float",
            )
        )
        dg = np.matmul(
            ...
            np.array(
                [[translation_vel.x], [translation_vel.y], [translation_vel.z]],
                dtype="float",
            ),
        )
...
if __name__ == "__main__":
    ...
    solution = robot.inverse_vel(g, forward.translation_vel)
    print("Solution: ")
    print(solution)
